...
bool CAssimpModel::LoadModelFromFile(char* sFilePath)
{
	...
	FOR(i, scene->mNumMeshes)
	{
		...
		FOR(j, iMeshFaces)
		{
			...
			FOR(k, 3)
			{
				...
				aiVector3D uv = mesh->HasTextureCoords(face.mIndices[k]) ? mesh->mTextureCoords[0][face.mIndices[k]] : aiVector3D(1.0f, 1.0f, 1.0f);
	...
	FOR(i, iNumMaterials)
	{
		...
		if (!ok)
		{
			std::string sPutToPath = sFilePath;
...
void CAssimpModel::RenderModel(GLenum RenderMode)
{
	...
	FOR(i, iNumMeshes)
	{
		...
		//glDrawElements(RenderMode, iMaterialIndices.size(), GL_UNSIGNED_INT,0);
